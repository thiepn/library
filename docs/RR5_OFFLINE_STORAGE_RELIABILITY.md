...
Chromium and Firefox use Playwright's native browser-context offline mode. Playwright/WebKit's native offline toggle can abort a document navigation internally before the page's service worker is allowed to satisfy the request, so RR5 does not treat that automation artifact as an application failure. The WebKit project instead runs through a qualification-only localhost origin proxy: normal requests are forwarded to the same Astro preview, and the test can deliberately return a deterministic `503 Service Unavailable` origin response while leaving the browser and production service worker running. RR5 treats origin `5xx` navigation responses as unavailable and falls back to the cached navigation, while cached application assets must remain fully styled and functional. This avoids the WebKit transport corruption caused by hard-resetting the localhost socket while still proving that the production service worker, not the origin, satisfies the offline journey. The proxy is test-only and is never included in the production artifact.
