...
if (present) {
  ...
  pass('EPUB_READER_A11Y_EPUB_FRAME', a11y.includes('frame.title =') && a11y.includes("getAttribute('xml:lang')") && a11y.includes('data-reader-a11y-style') && a11y.includes(':focus { outline: 3px solid currentColor'), 'EPUB iframes are named and receive language/focus/reduced-motion accessibility hardening');
  ...
  pass('EPUB_READER_A11Y_REDUCED_MOTION', a11y.includes("matchMedia('(prefers-reduced-motion: reduce)')") && css.includes('@media (prefers-reduced-motion: reduce)') && css.includes('animation-duration: 0.01ms !important'), 'Reduced-motion preference is represented in runtime state and enforced in shell/EPUB content');
  pass('EPUB_READER_A11Y_FORCED_COLORS', a11y.includes("matchMedia('(forced-colors: active)')") && css.includes('@media (forced-colors: active)') && css.includes('outline: 3px solid Highlight !important'), 'Forced-colors mode preserves system focus and boundary colors');
  pass('EPUB_READER_A11Y_HIGH_CONTRAST', a11y.includes("matchMedia('(prefers-contrast: more)')") && css.includes('@media (prefers-contrast: more)') && css.includes('--reader-shell-rule: var(--reader-shell-secondary)'), 'Higher-contrast preference strengthens separators and panel boundaries');
  ...
  pass('EPUB_READER_A11Y_EXISTING_FOUNDATIONS', shellComponent.includes('reader-skip-link') && shellComponent.includes('role="alert"') && shellComponent.includes('aria-busy="true"') && desktopCss.includes('@media (hover: hover) and (pointer: fine)'), 'Skip navigation, busy/error semantics, and input-modality foundations remain present');
